...
class KerasPredictionService:
    ...
    def _sync_predict(self, rainfall: float, upstream: float) -> float:
        # Use model.predict_on_batch for optimized single predictions.
        # This ensures compiled model scaling layers run correctly, avoiding raw forward pass quirks.
        input_data = np.array([[rainfall, upstream]], dtype=np.float32)
        predicted_batch = self.model.predict_on_batch(input_data)
        return float(predicted_batch[0][0])

    async def predict_async(self, rainfall: float, upstream: float) -> float:
        """
        Runs neural network inference in a non-blocking asynchronous worker thread.
        """
        return await asyncio.to_thread(self._sync_predict, rainfall, upstream)
